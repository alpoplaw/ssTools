...
simout<-mclapply(1:100, function(arg2){
    DESeqout<-lapply(2:30, function(arg){
    set.seed(arg*arg2)
    counts<-sample(e,arg*2, replace=FALSE)
    b<-sample(ne,arg, replace=FALSE)
    counts[sG.names2,(1:arg)]<-b[sG.names2,]
    condition<-as.factor(cbind(c(rep(0,arg), c(rep(1,arg)))))
    d <- newCountDataSet(counts, condition )
    d <- estimateSizeFactors( d )
    d <- estimateDispersions( d )
    d <- nbinomTest(d, "0", "1")
    return(d)})
    DESeqout},mc.cores=ncores)
