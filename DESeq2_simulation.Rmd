...
setwd("PATH to your working direction/DESeq2/")
library(edgeR)
library(parallel)
...
simout<-mclapply(1:100, function(arg2){
  DESeq2out<-lapply(2:30, function(arg){
    set.seed(arg*arg2)
    ...
    pheno<-as.data.frame(cbind(c(rep("contol",arg), c(rep("test",arg)))))
    row.names(pheno)<- colnames(counts)
    colnames(pheno)<- "condition"
    d <- DESeqDataSetFromMatrix(countData=counts, colData=pheno, design = ~ condition )
    d <- DESeq(d)
    res <-results(d)
    return(res)})
  DESeq2out},mc.cores=ncores)
